...
export default function Workspace({ me, onSignedOut }: { me: Me; onSignedOut: () => void }) {
  ...
  return (
    ...
    <div className="mx-auto flex h-full max-w-[1280px] flex-col bg-paper shadow-[0_0_0_1px_var(--color-line)]">
      <Topbar workspaceName={me.workspace.name} onSwitched={() => window.location.assign('/')} email={me.user.email} name={me.user.name} isAdmin={me.isAdmin} view={view} onView={setView} onSearch={doSearch} onNew={() => setAdd({ tab: 'write', layer: layerOf(path ?? 'wiki/x') || 'wiki' })}
