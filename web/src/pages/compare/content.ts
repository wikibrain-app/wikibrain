...
export const compareDocs: CompareDoc[] = [
  ...
  },
  {
    ...
    rows: [
      ...
      r(['圖譜、反向連結、版本', 'Graph, backlinks, versions'], ['圖譜與反向連結原生；版本靠 Sync 或 git', 'Graph and backlinks are native; versions via Sync or git'], ['都有：圖譜（篩選、時間軸）、反向連結、每次儲存的版本與回滾', 'All included: graph (filters, timeline), backlinks, a version per save with rollback']),
  ...
  },
  {
    ...
    rows: [
      ...
      r(['圖譜與版本', 'Graph and versions'], ['無圖譜；30 天垃圾桶版本', 'No graph; 30-day trash versions'], ['圖譜（篩選、時間軸）、每次儲存的版本、回滾', 'Graph (filters, timeline), a version per save, rollback']),
