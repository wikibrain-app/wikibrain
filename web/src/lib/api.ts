...
export interface ChatMessage { role: 'user' | 'assistant'; content: string; at: string; failed?: boolean; jobId?: number; steps?: number; tokens_in?: number; tokens_out?: number; cost_usd?: number | null; tools?: { tool: string; path?: string; query?: string; failed?: boolean }[]; filedTo?: string }
...
export const api = {
  ...
  workspaces: () => request<{ workspaces: { id: string; name: string; lang: Lang; created_at: string }[]; current: string; limit: number }>('GET', '/api/workspaces'),
  createWorkspace: (name?: string) => request<{ id: string; name: string }>('POST', '/api/workspaces', { name }),
  switchWorkspace: (id: string) => request<{ id: string; name: string }>('PUT', '/api/me/workspace', { id }),
  renameWorkspace: (id: string, name: string) => request<{ id: string; name: string }>('PATCH', `/api/workspaces/${id}`, { name }),
  deleteWorkspace: (id: string, confirm: string) => request<{ ok: true }>('DELETE', `/api/workspaces/${id}?confirm=${encodeURIComponent(confirm)}`),
