  ...
  return (
    <ToastCtx.Provider value={{ toast }}>
      ...
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex w-[min(640px,calc(100%-2rem))] flex-col-reverse gap-2 pointer-events-none" role="status" aria-live="polite">
        {items.map(t => (
          <div key={t.id} className={`toast pointer-events-auto flex items-center gap-3 rounded-[10px] px-4 py-2.5 text-[13px] text-white shadow-lg ${t.kind === 'error' ? 'bg-[#8A3B2E]' : 'bg-ink'}`}>
            <span>{t.message}</span>
            {t.action && (
